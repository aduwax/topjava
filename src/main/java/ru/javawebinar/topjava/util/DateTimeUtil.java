...
public class DateTimeUtil {
    ...
    public static <T extends Comparable<T>> boolean isBetweenHalfOpen(T lt, T startInterval, T endInterval) {
        if (lt instanceof LocalDate) {
            return lt.compareTo(startInterval) >= 0 && lt.compareTo(endInterval) <= 0;
        }
        return lt.compareTo(startInterval) >= 0 && lt.compareTo(endInterval) < 0;
    }
